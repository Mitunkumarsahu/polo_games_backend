...
def find_lowest_available_id(db: Session) -> int:
    try:
        existing_ids = [entry.id for entry in db.query(SocialMedia.id).order_by(SocialMedia.id).all()]
...
@socialmedia_router.post("/create_items/", response_model=ImageLinkResponse)
def create_item(link: str = Form(...), image: UploadFile = File(...), db: Session = Depends(get_db), current_user: dict = Depends(verify_role(["Admin", "Superadmin"]))):
    try:
        ...
        new_item = SocialMedia(id=new_id, link=link, image=image_data)  
        db.add(new_item)
        db.commit()
        db.refresh(new_item)
...
@socialmedia_router.get("/items/", response_model=List[ImageLinkResponse])
...
@socialmedia_router.get("/items/{item_id}", response_model=ImageLinkResponse)
