...
def find_lowest_available_id(db: Session) -> int:
    try:
        existing_ids = [entry.id for entry in db.query(ImageLinkBackup.id).order_by(ImageLinkBackup.id).all()]
...
@image_link_router_for_backup.post("/create_items/", response_model=ImageLinkResponse)
def create_item(link: str = Form(...), image: UploadFile = File(...), db: Session = Depends(get_db), current_user: dict = Depends(verify_role(["Admin", "Superadmin"]))):
    try:
        ...
        new_item = ImageLinkBackup(id=new_id, link=link, image=image_data)  
        db.add(new_item)
        db.commit()
        db.refresh(new_item)
...
@image_link_router_for_backup.get("/items/", response_model=List[ImageLinkResponse])
def read_items(db: Session = Depends(get_db)):
    try:
        items = db.query(ImageLinkBackup).all()
        response_items = []
        for item in items:
...
@image_link_router_for_backup.get("/items/{item_id}", response_model=ImageLinkResponse)
def read_item(item_id: int, db: Session = Depends(get_db)):
    try:
        item = db.query(ImageLinkBackup).filter(ImageLinkBackup.id == item_id).first()
        if not item:
...
@image_link_router_for_backup.put("/items/{item_id}", response_model=ImageLinkResponse)
...
@image_link_router_for_backup.delete("/items/{item_id}", response_model=dict)
